...
for isub=1:8
    ...
    for visualRegion=visualRegions
        ...
        for roinum=1:length(rois)
            ...
            residuals_model2_orth = NaN(nsplits, nvox(roinum),maxNumTrials);
            ...
            for isplit=1:nsplits
                ...
                for ivox=1:nvox(roinum)
                    ...
                    pred_model1{roinum}(isplit,ivox,:) = squeeze(voxOriCoef1{roinum}(isplit,ivox,:))'*voxPrfOriSample1';
                    ...
                    coef_model2_orth{roinum}(isplit,ivox,:) = voxPrfOriSample2 \ residuals_model1{roinum}(isplit,ivox,1:numTrials);
                    pred_model2_orth{roinum}(isplit,ivox,:) = coef_model2_orth{roinum}(isplit,ivox,:)*voxPrfOriSample2;
                    residuals_model2_orth{roinum}(isplit,ivox,1:numTrials) =  residuals_model1{roinum}(isplit,ivox,1:numTrials) - pred_model2_orth{roinum}(isplit,ivox,:);
                    ...
                    R2_model2_orth{roinum}(isplit,ivox) = rsquared(residuals_model2_orth{roinum}(isplit,ivox,1:nsum(splitImgTrials(isplit,:))),residuals_model1{roinum}(isplit,ivox,1:numTrials));

                    % Step 4: Shared Variance (if needed)
                    pred_combined{roinum}(isplit,ivox,:) = pred_model1{roinum}(isplit,ivox,:) + pred_model2_orth{roinum}(isplit,ivox,:);
                    residuals_combined{roinum}(isplit,ivox,:) = voxBetas - pred_combined{roinum}(isplit,ivox,:);
                    R2_combined(isplit,ivox) = rsquared(residuals_combined{roinum}(isplit,ivox,:),roiBetas{roinum}(ivox,imgTrials>0));
