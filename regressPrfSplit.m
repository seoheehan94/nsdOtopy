...
for visualRegion=visualRegions
    ...
    for roinum=1:length(rois)
        ...
        voxOriCoef{roinum} = zeros(nsplits, nvox(roinum),3*numOrientations+1);
        voxCoef{roinum} = zeros(nsplits, nvox(roinum),numLevels+1);
        voxPredOriCoef{roinum} = zeros(nsplits, nvox(roinum),3*numOrientations+1);
        voxPredCoef{roinum} = zeros(nsplits, nvox(roinum),numLevels+1);
        voxOriPredOriCoef{roinum} = zeros(nsplits, nvox(roinum),3*numOrientations+1);
        voxResidOriCoef{roinum} = zeros(nsplits, nvox(roinum),3*numOrientations+1);
        voxOriResidOriCoef{roinum} = zeros(nsplits, nvox(roinum),3*numOrientations+1);
...
%     synth.voxOriResidual = voxOriResidual;
    ...
    save(fullfile(boxfolder,['regressPrfSplit_bottomsf' bandpassStr '_v' num2str(visualRegion) '_sub' num2str(isub) '.mat']), ...
        'nsd',...
