...
prfSampleLevLen = cell(length(rois),1);
...
for roinum=1:length(rois)
    ...
    prfSampleLevLenRoi = zeros(length(allImgs),nvox,numLevels, numLengths);
    
    %loop through  images
    parfor iimg=1:length(allImgs)
        ['sub: ' num2str(isub) ', roi: ' num2str(iroi) ', image: ' num2str(iimg)]
        imgNum = allImgs(iimg);
        lenfilename = ['lenImg' num2str(imgNum) '.mat'];
        data = load(fullfile(lenfolder, lenfilename),'modelLen');
        imgPrfSampleLevLen = zeros(nvox,numLevels,numLengths);
        %loop through voxels
        for ivox=1:nvox
            %sample for each ori level (summed across orientations!)
            voxPrfSampleLevLen = zeros(numLevels,numLengths);
            %create the pRF for this voxel
            G = exp(-((X-x0(ivox)).^2+(Y-y0(ivox)).^2)/(2*sz(ivox)^2));
            
            for ilev = 1:numLevels
                voxPrfSampleLen = zeros(numLengths,1);
                for ilen=1:numLengths
                    temp = data.modelLen(ilen,:,:);
                    voxPrfSampleLen(ilen) = temp(:)'*G(:);
                end
                voxPrfSampleLevLen(ilev,:) = voxPrfSampleLen;
            end
            imgPrfSampleLevLen(ivox,:,:) = voxPrfSampleLevLen;
        end
        prfSampleLevLenRoi(iimg,:,:,:) = imgPrfSampleLevLen;
    end
    prfSampleLevLen{roinum} = prfSampleLevLenRoi;

    prffolder = '/bwlab/Users/SeoheeHan/NSDData/rothzn/nsd/Length/prfsample_Len/';
    save(fullfile(prffolder,['prfSampleStim_len_v' num2str(visualRegion) '_sub' num2str(isub) '.mat']),'prfSampleLevLen',...
...
% save(fullfile(prffolder,['prfSampleStim_v' num2str(visualRegion) '_sub' num2str(isub) '.mat']),'prfSampleLevLen','prfSampleLev',...
%     'rois','allImgs','numLevels','numLengths','interpImgSize','backgroundSize','pixPerDeg',...
%     'roiPrf','-v7.3');
delete(g);
