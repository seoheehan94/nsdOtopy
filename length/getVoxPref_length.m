...
function getVoxPref_length(isub,numregions)
...
numLengths = 8;
figFolder = ['/bwlab/Users/SeoheeHan/NSDData/rothzn/nsd/Length/figures/'];
nperms=1000;
...
gratings = load(['gratings_length.mat'],'numLengths','numLevels',...
...
%%
    function prefAngle = gratingPrefLen(fullCoef,modelLenEnergy);
        [numFreqs, numAngles, numLevels, numLengths] = size(modelLenEnergy);
        numvox = size(fullCoef,1);
        coefMat = reshape(fullCoef,numvox,numLevels,numLengths);%vox x levels x orientations
        coefVec = reshape(fullCoef,numvox,numLevels*numLengths);%vox x coefficients
        modelEnergy = reshape(modelLenEnergy,numFreqs*numAngles,numLevels*numLengths);
